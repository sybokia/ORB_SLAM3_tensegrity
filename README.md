...
'''
9. For any mode, after closing the node, the result 'KeyFrameTrajectory.txt' in TUM format is save at where the command was executed. The [evo](https://github.com/MichaelGrupp/evo) toolkit provides convinent ways to evaluate the results.
...
- ``` shell
  ...
  ```
### Exp 2: Go-SLAM Image-level Perturbation (Dynamic Perturbation) Evaluation on Replica [1108 Update]
- Nested script:
- ``` shell
...
- Expanded script:
- ``` shell
  bash ./scripts/run_replica_goslam_rgbd_img_perturb_dynamic_expand.sh
  bash ./scripts/run_replica_goslam_mono_img_perturb_dynamic_expand.sh
  ```
### Exp 3: Go-SLAM Motion Perturbation Evaluation on Replica [1108 Update]
- Nested script:
- ``` shell
...
- Expanded script:
- ``` shell
  bash ./scripts/run_replica_goslam_rgbd_faster_motion_expand.sh
  bash ./scripts/run_replica_goslam_mono_faster_motion_expand.sh
  ```
